...
def objective_factory(features, target, nan_indices, nan_n, feature_names):
    ...
    def fitness(solution):
        ...
        # Impute missing values with solution
        for k, (row, col) in enumerate(nan_indices):
            temp_features[row, col] = solution[k]
        
        # Simplified cross-validation - use just 1 fold for faster execution
        train_idx, test_idx = next(skf.split(features_clean, target_clean))
        X_train, y_train = features_clean[train_idx], target_clean[train_idx]
        
        # Train classifier
        classifier.fit(X_train, y_train)
        
        # Predict using full imputed dataset
        y_pred = classifier.predict(temp_features)
        
        # Evaluate accuracy only on clean test set
        test_mask = np.zeros(len(temp_features), dtype=bool)
        test_mask[test_idx] = True
        clean_test_mask = np.logical_and(test_mask, nan_mask)
        
        if np.sum(clean_test_mask) > 0:
            acc = accuracy_score(target[clean_test_mask], y_pred[clean_test_mask])
        else:
            acc = 0.5
        
        return 1 - acc  # since mealpy looks for minimum, use (1 - accuracy)
    
    return fitness


def run_optimizers(features, target, nan_indices, nan_n, feature_names, lb, ub, epochs=10, pop_size=20):
    ...
    for opt in optimizers:
        ...
        termination = {
            "max_epoch": epochs  # Stop after this many epochs
        }
        
        # Solve problem with termination condition
        result = opt["algorithm"].solve(problem, termination=termination)
        
        # Calculate accuracy (1 - fitness)
    ...
    # Select best result based on accuracy
...
def optimize_and_evaluate(path, epochs=10, pop_size=20):
